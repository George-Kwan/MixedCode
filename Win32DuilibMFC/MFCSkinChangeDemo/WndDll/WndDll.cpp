...
LRESULT CALLBACK HOOKProc(int nCode,WPARAM wParam,LPARAM lParam)
{
	...
	if(hWnd)
	{
		...
		*/
		if(_tcscmp/*strcmp*/(ClassName,_T("Button"))==0)//修改按钮的窗口函数
		{
			CWnd* pWnd=CWnd::FromHandle(hWnd);
		...
		*/
		else if(_tcscmp/*strcmp*/(ClassName,_T("Edit"))==0)
		{
			CWnd* pWnd=CWnd::FromHandle(hWnd);
		...
		关联的CDrawDialog对象
		*/
		else if(_tcscmp/*strcmp*/(ClassName,_T("#32770"))==0)	//vc程序默认对话框的类名为#32770
		{
