...
// 滚动条的初始化，设置前先从资源向导中开启滚动条
// 参数一：控件句柄,普通的用GetDlgItem获取，List直接使用关联控件
// 参数二：滚动条的位图资源文件
//----------------------------------------------------------
#ifndef LONG_PTR
...
//////////////////////////////////////////////////
//
//		VC6.0 的RichEdit 类名是RICHEDIT
//
//////////////////////////////////////////////////
	GetClassName(hwnd, szClassName, sizeof(szClassName));
	#if _MSC_VER <=1200
		if( _strcmpi(szClassName, TEXT("RICHEDIT")) == 0 ||
			_strcmpi(szClassName, TEXT("RichEdit20A")) == 0 ) {
			psb->fRichEdit = TRUE;
		}
	#else
...
BOOL SkinSB_GetSizeBoxRect(LPSKINSB psb, LPRECT lprc)
{
	...
	if( (dwStyle & WS_HSCROLL) && (dwStyle & WS_VSCROLL) ) {
		...
		if( psb->fLeftScrollBar ) {
			lprc->left = rect.left - DEFAULTSIZE;//SBMETRICS::cxVArrow;
			lprc->right = rect.left;
		}
		else {
			lprc->left = rect.right;
			lprc->right = rect.right + DEFAULTSIZE;//SBMETRICS::cxVArrow;
		}

		lprc->top   = rect.bottom;
		lprc->bottom = rect.bottom + DEFAULTSIZE;//SBMETRICS::cxHArrow;
		return TRUE;
	}
	return FALSE;
}
...
BOOL SkinSB_GetScrollBarRect(LPSKINSB psb, BOOL fVert, LPRECT lprc)
{
	...
	if( fVert ) {
		if( psb->fLeftScrollBar ) {
			...
			if( dwStyle & WS_VSCROLL )
				lprc->left -= DEFAULTSIZE;//SBMETRICS::cxThumb;
		}
		else {
			lprc->right = lprc->left = rect.right;
			if( dwStyle & WS_VSCROLL )
				lprc->right += DEFAULTSIZE;//SBMETRICS::cxThumb;
		}
		lprc->top = rect.top;
		...
		if( dwStyle & WS_HSCROLL )
			lprc->bottom += DEFAULTSIZE;//SBMETRICS::cyThumb;
		lprc->left = rect.left;
...
void SkinSB_DrawScrollBar(LPSKINSB psb, HDC hdc, BOOL fVert)
{
	...
	int nSaveDC = 0;
	//fVert=0为横条 1为竖条
	SkinSB_GetGrooveRect(psb, &rcGroove, fVert);//滚动条的长度
	SkinSB_GetThumbRect(psb, &rcThumb, fVert);//滚动方块
...
///////////////////////
	...
	// Check scrollbar of size whether valid  检查滚动条大小是否合法
	...
	if( sbc.pxThumbBottom < sbc.pxDownArrow ) {
		...
		OffsetRect(&rc, -rect.left, -rect.top);//滚动条的白色与区域
	//////////////
	#if _DEBUG
	sprintf(szText, "滚动条的白色和方块:Left=%d Top=%d Width=%d Heigh=%d\n", rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top);
	printf("%s\n",szText);
	#endif
	///////////
		SkinSB_DrawGroove(psb, hdc, &rc, fVert);
	...
	rcMemDC.left = 0;
	rcMemDC.top = 0;
	rcMemDC.right = rc.right-rc.left;
	rcMemDC.bottom = rc.bottom-rc.top;
	
/////////////////////// 这里哦
#if _DEBUG
sprintf(szText, "rcMemDC:Left=%d Top=%d Width=%d Heigh=%d\n", rcMemDC.left, rcMemDC.top, rcMemDC.right - rcMemDC.left, rcMemDC.bottom - rcMemDC.top);
printf("%s\n",szText);
#endif
///////////////////////
	// Create memory DC
...
printf("%s\n",szText);
		...
		////////////////////
		#if _DEBUG
				sprintf(szText, "横行滚动轴长度cx = %d\n",cx);
				printf("%s\n",szText);
		#endif
		///////////////////////
		//
		//	找了半天才找到，是这里的一个ii写成了i。 1:00 2016/5/1
		//
		////////////////////////////
		//for( int i = 4; i < cx - 4; i += 8 )
		for(; ii < cx - 4; ii += 8 )
			BitBlt(memDC, ii, 0, 8, cy, hSrcDC, xOffset + 4, yOffset, SRCCOPY);
		...
		if( cx > 16 + 8 ) {
			//int x = (cx - 16) / 2;
			 xx = (cx - 16) / 2;
			BitBlt(memDC, xx, 0, 16, cy, hSrcDC, xOffset1, yOffset1, SRCCOPY);
		}
	}
	// 将内存DC输出喽!
...
#if _DEBUG
	///////////////////////
	sprintf(szText, "Left=%d Top=%d Width=%d Heigh=%d\n", rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top);
	printf("%s\n",szText);
	//////////////////
#endif
...
void SkinSB_SBCalc(LPSKINSB psb, LPSBCALC lpcalc, BOOL fVert)
{
	...
	if( fVert ) {
		psi = &psb->Vert;
		nArrowsize   = 16;//SBMETRICS::cxVArrow;
		...
		psi = &psb->Horz;
		nArrowsize   = 16;//SBMETRICS::cxHArrow;
...
UINT SkinSB_GetState(LPSKINSB psb, BOOL fVert, UINT nHitItem)
{
	...
	// 这里修改了
	if( (nHitItem == (UINT)GET_HTITEM(psb->dwLastCode)) && (fVert == (BOOL)GET_HTVERT(psb->dwLastCode)) )
		fHotTracked = TRUE;
...
LRESULT SkinSB_OnNcCalcSize(LPSKINSB psb, BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	...
	if( (dwStyle & WS_HSCROLL) && 
		(lprc->bottom - lprc->top) >16){//SBMETRICS::cxHArrow ) {
		lprc->bottom -= 16;//SBMETRICS::cxHArrow;
	}

	if( (dwStyle & WS_VSCROLL) &&
		(lprc->right - lprc->left) >16){// SBMETRICS::cxVArrow ) {
		if( psb->fLeftScrollBar )
			lprc->left += 16;//SBMETRICS::cxVArrow;
		else
			lprc->right -= 16;//SBMETRICS::cxVArrow;
	}
	return lr;
}
...
LRESULT SkinSB_OnMouseMove(LPSKINSB psb, WPARAM wParam, LPARAM lParam)
{
	...
	if( psb->fTracking && (wParam & MK_LBUTTON) ) {
		...
		lr = SkinSB_OnNcHitTest(psb, 0, dwPos);
